...
def _create_multi_meter_midi() -> pretty_midi.PrettyMIDI:
    ...
    for time, num, den in changes:
        pm.time_signature_changes.append(
            pretty_midi.TimeSignature(num, den, time)
        )
...
def _create_edge_case_midi() -> pretty_midi.PrettyMIDI:
    ...
    for i in range(100):
        ...
        note = pretty_midi.Note(velocity=vel, pitch=60 + (i % 12), start=7.0 + i / 100.0, end=7.0 + i / 100.0 + 0.01)
        piano.notes.append(note)
    
    # 正常部分（8-10sec）
    for i in range(20):
        note = pretty_midi.Note(velocity=80, pitch=60, start=8.0 + i / 10.0, end=8.0 + i / 10.0 + 0.1)
        piano.notes.append(note)
...
def test_quality_check_1_long_duration_stress(tmp_path: Path):
    ...
    for i in range(count):
        ...
        start = time.time()
        meta = extract_stage2_metadata(pm, chordmap_json=None)
...
def test_quality_check_2_multi_meter_backbeat(tmp_path: Path):
    ...
    for num, den, label in meters:
        pm = _create_test_midi(duration_sec=10.0, tempo=120.0, time_signature=(num, den), note_density=10)
        meta = extract_stage2_metadata(pm, chordmap_json=None)
...
def test_quality_check_6_determinism():
    """
    チェック6: 再現性テスト
    ...
    # 2回実行
    meta1 = extract_stage2_metadata(pm, chordmap_json=None)
    meta2 = extract_stage2_metadata(pm, chordmap_json=None)
    ...
    assert meta1["groove"]["backbeat_strength"] == meta2["groove"]["backbeat_strength"], "backbeat_strength 不一致"
...
def test_quality_check_controls_integrity_threshold():
    """
    ...
    meta_normal = extract_stage2_metadata(pm_normal, chordmap_json=None)
