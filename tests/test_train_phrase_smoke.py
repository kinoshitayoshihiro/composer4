...
def test_train_phrase(tmp_path: Path) -> None:
    ...
    assert ckpt.is_file()
    assert (tmp_path / "model.best.ckpt").is_file()
    assert (tmp_path / "hparams.json").is_file()
    assert (tmp_path / "metrics_epoch.csv").is_file()
    assert (tmp_path / "preds_preview.json").is_file()
    assert (tmp_path / "model.run.json").is_file()
