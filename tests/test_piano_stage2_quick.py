...
def gen() -> PianoGeneratorStage2:
    """Shared PianoGeneratorStage2 instance for Stage2 quick tests."""
    return PianoGeneratorStage2(use_stage2=True)
...
def test_melody_generation(gen):
    ...
    if melody_notes:
        print(
            f"  ✓ First note: pitch={melody_notes[0].pitch}, time={melody_notes[0].time:.2f}, vel={melody_notes[0].velocity}"
        )
        print(
            f"  ✓ Last note: pitch={melody_notes[-1].pitch}, time={melody_notes[-1].time:.2f}, vel={melody_notes[-1].velocity}"
        )
