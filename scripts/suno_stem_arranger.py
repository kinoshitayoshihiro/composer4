...
class SunoStemArranger:
    ...
    def _extract_onsets_ql(self, part: "stream.Part", dedupe_eps: float = 0.02) -> List[float]:
        ...
        try:
            ...
            def _pc_from_tonic(tn: "str|int") -> int:
                if isinstance(tn, int): return tn % 12
                try: return m21pitch.Pitch(str(tn)).pitchClass
            ...
            segs_pc = [{"bar": int(s.get("bar",0)), "tonic_pc": _pc_from_tonic(s.get("tonic", default_tpc)),
                        "mode": str(s.get("mode", key_mode))} for s in (segs or default_seg)]
            segs_pc.sort(key=lambda d: d["bar"])
            def _context_for_bar(bar_idx: int) -> tuple[int,set[int],int,str]:
                cur = segs_pc[0]
                for s in segs_pc:
                    if bar_idx >= s["bar"]: cur = s
                tpc = cur["tonic_pc"]; m = cur["mode"]; di = _mode_diatonic_pcs(tpc, m)
            ...
            def _score(pc: int, vel: int, bar_idx: int) -> float:
                ...
                if pcw_map and str((pc-tonic_pc)%12) in pcw_map:
                    ...
                    if pc == tonic_pc: s *= w_tonic
                    if pc == fifth_pc: s *= w_fifth
            ...
            # 量子化(任意)
            if qgrid:
                cand = [(round(off / qgrid) * qgrid, pc, midi, vel, b) for (off, pc, midi, vel, b) in cand]
            ...
            def _flush_group():
                ...
                candidate = None
                if (w_tonic, w_fifth, w_diat, w_non, vel_pow) == (1.0, 1.0, 1.0, 1.0, 0.0) and not pcw_map:
    ...
    def _derive_key_segments_from_chords(self, chords: List[str], bars: int,
                                         window_bars: int = 4, min_seg_bars: int = 4,
                                         mode_infer: str = "by_quality") -> List[Dict[str, Any]]:
        """
        ...
        segs = []
        cur_root = None; cur_mode = None; cur_start = 0
    ...
    def _derive_sections_from_profile(self, profile: Dict[str, Any], emotion: str, bars: int) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            out = []
            for s in secs:
                b = int(s.get("bar", 0)); lab = str(s.get("label", "") or "")
    ...
    def _extract_pitch_events(self, part: "stream.Part",
                              min_note_ql: float = 0.0,
                              quantize_grid: Optional[float] = None,
                              dedupe_eps: float = 0.02) -> List[List[float]]:
        """
            ...
            with open(csv_path, "r", newline="", encoding="utf-8") as f:
                ...
                for row in rdr:
                    if not row or row[0].strip().lower().startswith("offset"):
                        ...
                        continue
                    try:
                        off = float(row[0]); cls = str(row[1]).strip().lower()
                        ev.append([off, cls])
                    except Exception:
                        continue
        except Exception:
            ...
            with open(csv_path, "r", newline="", encoding="utf-8") as f:
                ...
                for row in rdr:
                    if not row or row[0].strip().lower().startswith("offset"):
                        continue
                    try:
                        off = float(row[0]); en = max(0.0, min(1.0, float(row[1])))
                        arr.append([off, en])
                    except Exception:
                        continue
        except Exception:
        ...
        try:
            ...
            if part_name in ['piano', 'guitar']:
                try:
                    ...
                    logger.warning("%s initialization with params failed: %s", part_name.capitalize(), e)
                    return None
    ...
    def _render_part(
        ...
        if isinstance(result, dict):
            logger.info(f"{name.capitalize()} returned dict with {len(result)} parts: {list(result.keys())}")
    ...
    def _insert_part_or_parts(
        ...
        if isinstance(result, list):
            ...
            for i, part in enumerate(result):
                ...
                logger.info(f"  ✅ {part_name.capitalize()} part {i+1}/{len(result)}: {note_count} notes")
            logger.info(f"✅ {part_name.capitalize()}: {total_notes} notes total ({len(result)} parts)")
            return total_notes
    ...
    def arrange_with_generators(
        ...
        logger.info(f"Emotion '{emotion}': humanize timing_ms={eparams['humanize']['timing_ms']}, vel_sigma={eparams['humanize']['vel_sigma']}")
        ...
        if "bass" in self.generators:
            ...
            try:
                ...
                if bass_preview is not None:
                    ...
                    onset_cfg = (((profile.get("emotions") or {}).get(emotion) or {}).get("mix_context") or {}).get("onset_extractor") or {}
                    # キー分節の自動推定（任意）
                    kd = (onset_cfg.get("key_detection") or {})
                    if bool(kd.get("enable", False)):
                        segs = self._derive_key_segments_from_chords(
                            chords=chords, bars=bars,
                    ...
                    bass_onsets = self._extract_onsets_ql(bass_preview, dedupe_eps=float(onset_cfg.get("dedupe_eps", 0.02)))
        ...
        try:
            ...
            drums_overrides = {
                ...
                "drums_params": (profile.get("emotions", {}).get(emotion, {}) or {}).get("drums_params"),
                # 追加：スタイルプリセット／ドラマー個性プリセットも透過
                "drums_style": (profile.get("emotions", {}).get(emotion, {}) or {}).get("drums_style"),
                "drummer_profile": (profile.get("emotions", {}).get(emotion, {}) or {}).get("drummer_profile"),
            }
            ...
            if "vocal" in self.generators:
                try:
                    ...
                    if vocal_prev is not None:
                        vcfg = (((profile.get("emotions") or {}).get(emotion) or {}).get("mix_context") or {}).get("vocal_extractor") or {}
                        self._onset_cfg = vcfg
                        vocal_onsets = self._extract_onsets_ql(vocal_prev, dedupe_eps=float(vcfg.get("dedupe_eps", 0.03)))
                        self._onset_cfg = {}
                        vocal_phrases = self._group_phrases(vocal_onsets, gap_ql=float(vcfg.get("phrase_gap_ql", 1.0)))
                        ...
                        vmeta = (((profile.get("emotions") or {}).get(emotion) or {}).get("mix_context") or {})
                ...
                if bass_part is None:
                    ...
                    if humanize:
                        self._apply_humanize(
                            bass_part, tempo_bpm=tempo, seed=seed,
        ...
        if "piano" in self.generators:
            ...
            try:
                ...
                if piano_part is None:
                    ...
                    if humanize:
                        self._apply_humanize(
                            piano_part, tempo_bpm=tempo, seed=seed,
                            timing_ms=eparams["humanize"]["timing_ms"],
                            vel_sigma=eparams["humanize"]["vel_sigma"]
                        )
            except Exception as e:
        ...
        if "guitar" in self.generators:
            ...
            try:
                ...
                if guitar_part is None:
                    ...
                    if humanize:
                        self._apply_humanize(
                            guitar_part, tempo_bpm=tempo, seed=seed,
                            timing_ms=eparams["humanize"]["timing_ms"],
                            vel_sigma=eparams["humanize"]["vel_sigma"]
                        )
            except Exception as e:
        ...
        if "strings" in self.generators:
            ...
            try:
                ...
                if strings_part is None:
                    ...
                    if humanize:
                        self._apply_humanize(
                            strings_part, tempo_bpm=tempo, seed=seed,
                            timing_ms=eparams["humanize"]["timing_ms"],
                            vel_sigma=eparams["humanize"]["vel_sigma"]
                        )
            except Exception as e:
...
def main():
    parser = argparse.ArgumentParser(
        description="Suno AI stem分離WAVから自動アレンジ"
    )
    parser.add_argument(
        '--input',
        type=Path,
