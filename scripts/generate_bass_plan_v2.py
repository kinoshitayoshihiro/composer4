#!/usr/bin/env python3
"""
generate_bass_plan_v2.py - Slot-based bass renderer for fill/riff system.

Architecture:
- Slot Planner: bars_with_slots.parquet (always_active: bass は常に演奏)
- Policy YAML: density/pattern_types/articulation (how to fire)
- Chord Source: manual_chordmap.json or chordmap_locked.json (root foundation)
...
def generate_bass_plan(
    ...
    """
    Main logic: Generate slot-based bass plan.
    ...
    if isinstance(pattern_types_cfg, list) and pattern_types_cfg:
        ...
        pattern_types = {"root_quarter": 0.6, "root_eighth": 0.25, "walking": 0.15}
    ...
    metadata = {
        ...
        "always_active": True,
        "generator": "generate_bass_plan_v2.py",
...
def main():
    parser = argparse.ArgumentParser(description="Generate bass plan (slot-based V2)")
    ...
    try:
        ...
        print(f"⚠️  RhythmAI unavailable: {exc}")
        rhythm_ai = None
    ...
    if DurationHumanizeAI is not None:
        try:
            duration_ai = DurationHumanizeAI(
                ...
                policy=policy,
                tempo_bpm=policy.get("global", {}).get("tempo_bpm", 120),
                rhythm_manifest_path=rhythm_ai.rhythm_manifest_path if rhythm_ai else None,
                vocab_instrument=args.rhythm_vocab or "bass",
            )
            duration_ai.annotate_plan(plan)
            plan.setdefault("metadata", {}).setdefault("ai_hooks", {})[
                "duration_humanize_ai"
            ] = True
