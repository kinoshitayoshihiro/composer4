...
if __name__ == "__main__":
    ...
    print(f"[INFO] Found {len(wav_files)} WAV files")
    print(f"[INFO] Conversion method: {args.method}")
    ...
    for wav_file in wav_files:
        try:
            ...
            if args.method == "basic":
                ...
                midi = convert_ensemble(wav_file, num_models=args.num_models)
            else:
            ...
            midi = post_process_midi(
                ...
                normalize_velocity=(not args.no_normalize_velocity),
            )
            
            # Save MIDI
            output_file = output_dir / f"{wav_file.stem}.mid"
            midi.write(str(output_file))
            
            # Save metadata
            meta_file = output_dir / f"{wav_file.stem}.json"
            metadata = {
                "source_wav": str(wav_file.name),
                "conversion_method": args.method,
                "num_notes": sum(len(inst.notes) for inst in midi.instruments),
                "duration": midi.get_end_time(),
                "onset_threshold": args.onset_threshold if args.method == "basic" else "ensemble",
                "frame_threshold": args.frame_threshold if args.method == "basic" else "ensemble",
            }
            with open(meta_file, "w") as f:
                json.dump(metadata, f, indent=2)
            
            print(f"✅ ({metadata['num_notes']} notes, {metadata['duration']:.1f}s)")
            stats["success"] += 1
    ...
    if stats["failed"] > 0:
        print("\n[WARNING] Some conversions failed. Check error messages above.")
