...
class SunoMIDIImporter:
    ...
    def import_and_enhance(
        self, input_path: Path, output_path: Path, skip_humanize: bool = False
    ) -> bool:
        """メインパイプライン実行"""
...
class SunoMIDIQualityChecker:
    ...
    def evaluate(self, original_path: Path, enhanced_path: Path):
        ...
        metrics["humanization"] = self.calc_humanization(original_path, enhanced_path)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Suno AI MIDI Import & Enhancement")
    ...
    parser.add_argument("--skip-humanize", action="store_true", help="Skip humanization step")
    parser.add_argument(
        "--evaluate", action="store_true", help="Evaluate quality after enhancement"
    )
    ...
    success = importer.import_and_enhance(args.input, args.output, skip_humanize=args.skip_humanize)
