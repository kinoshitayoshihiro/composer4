...
# -*- coding: utf-8 -*-
"""
apply_bass_dynamics_policy_v1_1.py

Provides a helper to apply a per‑bar dynamics policy for a Bass instrument.
...
dynamics parameters for the bass line, such as role (FOUNDATION, DRIVE,
...
`bass_dynamics` dictionary to each supplied context and emit a JSON plan for
debugging.
"""
...
class BassDynamicsDecision:
    ...
    role: str = "FOUNDATION"
    root_anchor_bias: float = 0.7
    pedal_point_bias: float = 0.2
    passing_tone_density: float = 0.08
    octave_motion_bias: float = 0.06
    syncopation_bias: float = 0.08
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "role": self.role,
            "root_anchor_bias": self.root_anchor_bias,
            "pedal_point_bias": self.pedal_point_bias,
            "passing_tone_density": self.passing_tone_density,
            "octave_motion_bias": self.octave_motion_bias,
            "syncopation_bias": self.syncopation_bias,
...
class BassDynamicsPolicyV11:
    ...
    def decide_for_bar(
        ...
        root_anchor_bias = float(self.defaults.get("root_anchor_bias", 0.7))
        pedal_point_bias = float(self.defaults.get("pedal_point_bias", 0.2))
        passing_tone_density = float(self.defaults.get("passing_tone_density", 0.08))
        octave_motion_bias = float(self.defaults.get("octave_motion_bias", 0.06))
        syncopation_bias = float(self.defaults.get("syncopation_bias", 0.08))
        ...
        root_anchor_bias = float(sec_blob.get("root_anchor_bias", root_anchor_bias))
        pedal_point_bias = float(sec_blob.get("pedal_point_bias", pedal_point_bias))
        passing_tone_density = float(sec_blob.get("passing_tone_density", passing_tone_density))
        octave_motion_bias = float(sec_blob.get("octave_motion_bias", octave_motion_bias))
        syncopation_bias = float(sec_blob.get("syncopation_bias", syncopation_bias))
        ...
        if vb == "NEG" and "valence_NEG" in self.axis_modifiers:
            ...
            pedal_point_bias += float(mod.get("pedal_point_add", 0.0))
            syncopation_bias += float(mod.get("syncopation_add", 0.0))
        ...
        if vb == "POS" and "valence_POS" in self.axis_modifiers:
            ...
            playfulness_bias += float(mod.get("playfulness_add", 0.0))
            pedal_point_bias += float(mod.get("pedal_point_add", 0.0))
            octave_motion_bias += float(mod.get("octave_motion_add", 0.0))
        ...
        if ar == "HIGH" and "arousal_HIGH" in self.axis_modifiers:
            ...
            passing_tone_density += float(mod.get("passing_tone_add", 0.0))
            syncopation_bias += float(mod.get("syncopation_add", 0.0))
        ...
        if self.low_guard.get("enable", False):
            ...
            if energy <= float(cond.get("energy_max", 0.35)):
                ...
                passing_tone_density = float(enf.get("passing_tone_density", passing_tone_density))
                octave_motion_bias = float(enf.get("octave_motion_bias", octave_motion_bias))
                syncopation_bias = float(enf.get("syncopation_bias", syncopation_bias))
                pedal_point_bias = float(enf.get("pedal_point_bias", pedal_point_bias))
        ...
        if self.edge_rules.get("enable", False) and is_edge:
            ...
            passing_tone_density += float(pass_add_map.get(ar, 0.0))
            syncopation_bias += float(sync_add_map.get(ar, 0.0))
        ...
        root_anchor_bias = clamp_param("root_anchor_bias", root_anchor_bias)
        pedal_point_bias = clamp_param("pedal_point_bias", pedal_point_bias)
        passing_tone_density = clamp_param("passing_tone_density", passing_tone_density)
        octave_motion_bias = clamp_param("octave_motion_bias", octave_motion_bias)
        syncopation_bias = clamp_param("syncopation_bias", syncopation_bias)
        playfulness_bias = clamp_param("playfulness_bias", playfulness_bias)

        return BassDynamicsDecision(
            bar_index=bar_index,
            section=section_id,
            ...
            arousal_bucket=ar,
            role=role,
            root_anchor_bias=root_anchor_bias,
            pedal_point_bias=pedal_point_bias,
            passing_tone_density=passing_tone_density,
            octave_motion_bias=octave_motion_bias,
...
def apply_bass_dynamics_policy_v1_1(
    contexts: List[Any],
    policy_yaml_path: str = "config/bass_dynamics_policy_v1_1.yaml",
    ...
    """
    Apply the bass dynamics policy to a list of BarContext-like objects.
    Each context receives a `bass_dynamics` dictionary and a plan JSON is
    ...
    """
    policy = BassDynamicsPolicyV11(policy_yaml_path)

    # YAML outputs優先参照（レビュー対応：問題2）
    if out_plan_path is None:
        yaml_outputs = policy.policy.get("outputs", {})
        out_plan_path = yaml_outputs.get("per_bar_dynamics_json", "plans/bass_dynamics_plan.json")
    ...
    decisions: List[BassDynamicsDecision] = []
    for i, ctx in enumerate(contexts):
        ...
        setattr(
            ...
            {
                "role": dec.role,
                "root_anchor_bias": dec.root_anchor_bias,
                "pedal_point_bias": dec.pedal_point_bias,
                "passing_tone_density": dec.passing_tone_density,
                "octave_motion_bias": dec.octave_motion_bias,
                "syncopation_bias": dec.syncopation_bias,
    ...
    plan = {
        "version": "1.1",
        "policy_id": str(policy.policy.get("policy_id", "BASS_DYN_V1_1")),
        "instrument": "BASS",
