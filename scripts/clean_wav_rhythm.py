...
class RhythmWAVConfig:
    ...
    # Onset検出
    onset_envelope: str = "rms"  # 'rms' or 'spectral_flux'
    hop_length: int = 512
...
def extract_metadata_from_filename(filename: str) -> Dict[str, Any]:
    ...
    """
    metadata = {"pattern_id": None, "style": None, "bpm": None, "time_sig": None}
...
def validate_wav_file(
    wav_path: Path, config: RhythmWAVConfig
    ...
    try:
        ...
        onset_env = librosa.onset.onset_strength(
            y=y, sr=sr, hop_length=config.hop_length, aggregate=np.median
        )

        onsets = librosa.onset.onset_detect(
            onset_envelope=onset_env, sr=sr, hop_length=config.hop_length, backtrack=True
        )
        ...
        tempo, beats = librosa.beat.beat_track(
            onset_envelope=onset_env, sr=sr, hop_length=config.hop_length
        )
        ...
        metadata = {
            ...
            "onset_times": librosa.frames_to_time(
                onsets, sr=sr, hop_length=config.hop_length
            ).tolist(),
        }
...
class RhythmWAVShardWriter:
    ...
    def add(self, file_id: str, metadata: Dict[str, Any]):
        """レコード追加"""
        self.current_records.append({"file_id": file_id, "metadata": metadata})

        self.index[file_id] = {"shard": self.current_shard, "index": len(self.current_records) - 1}
    ...
    def process(self) -> Dict[str, Any]:
        ...
        with ProcessPoolExecutor(max_workers=self.jobs) as executor:
            ...
            for future in tqdm(as_completed(futures), total=len(futures), desc="Validating"):
                ...
                results.append(
                    {"path": wav_file, "valid": valid, "reason": reason, "metadata": metadata}
                )
        ...
        for r in tqdm(failed, desc="Quarantining"):
            ...
            with open(error_log, "w", encoding="utf-8") as f:
                json.dump({"file": str(rel_path), "reason": r["reason"]}, f, indent=2)
...
def main():
    ...
    parser.add_argument(
        "--in", dest="input_dir", type=Path, required=True, help="Input directory (rhythmAI/groove)"
    )
    parser.add_argument("--out", type=Path, required=True, help="Output directory (cleaned)")
    parser.add_argument("--quarantine", type=Path, required=True, help="Quarantine directory")
    parser.add_argument("--pickle-out", type=Path, required=True, help="Pickle output directory")
