#!/usr/bin/env python3
"""
Bass Pattern Recommender - v3互換パターン推薦システム

Phase 26: 他楽器ML展開 - Bass ML基盤
...
- Safe-Kit（基本ルートパターン）フォールバック
- KPI Gates維持（root_hit_rate ≥ 0.85, chord_fit ≥ 0.70）
...
Usage:
    ...
    rec = BassPatternRecommender(
        ...
        model_pickle_path="ml/stage2_bass_v3_ml.pickle"  # Optional
    )
...
class BassQuery:
    ...
    Attributes:
        ...
        groove_style: グルーブスタイル（straight/swing/shuffle）
    """
    tempo_bpm: float
    ...
    groove_style: str = "straight"  # straight/swing/shuffle


@dataclass
class BassRecommendResult:
    """推薦結果
    ...
    safety_reason: str
    root_hit_rate: float = 0.0
    chord_fit: float = 0.0
    density: float = 0.0


class BassPatternRecommender:
    """ベースパターン推薦システム（v3互換 + ML推論）
    
    v3思想:
    - Top-1確率を直接採用（閾値判定のみ）
    - ML推論（XGBoost/LogReg）でPattern Family予測
    - 低確率/低マージン時はSafe-Kitへフォールバック
    - KPI Gates維持（root_hit_rate, chord_fit, density）
    """
    ...
    def __init__(
        ...
        if safe_kit_path:
            ...
            if safe_kit_path.exists():
                ...
                logger.info(f"Loaded Bass Safe-Kit from {safe_kit_path}")
            else:
                logger.warning(f"Bass Safe-Kit not found: {safe_kit_path}")
        ...
        if model_pickle_path:
            ...
            if model_pickle_path.exists():
                try:
                    ...
                    logger.info(f"Loaded Bass ML model from {model_pickle_path}")
                    logger.info(f"Model type: {type(self.ml_model).__name__}")
                except Exception as exc:
                    logger.error(f"Failed to load Bass ML model: {exc}")
            else:
                logger.warning(f"Bass ML model pickle not found: {model_pickle_path}")
    ...
    def _build_index(
        ...
        Family/Tempo/Section/Groove別にパターンIDをグループ化
        ...
        for pid, ptn in patterns.items():
            ...
            # Tempo別インデックス（40 BPM刻み - ベースは広範囲）
            tempo_hint = ptn.get("tempo_hint", 120)
            tempo_bucket = (int(tempo_hint) // 40) * 40
            ...
            # Groove別インデックス
            groove_hint = ptn.get("groove_hint", "straight")
            key_groove = f"groove:{groove_hint}"
            index.setdefault(key_groove, []).append(pid)
        
        logger.info(f"Built Bass pattern index: {len(index)} keys, {len(patterns)} patterns")
        return index
    ...
    def _extract_features(
        ...
        features.append(1.0 if query.tempo_bpm < 80 else 0.0)  # is_slow
        features.append(1.0 if query.tempo_bpm > 140 else 0.0)  # is_fast
        ...
        # Groove Style特徴量（One-Hot）
        groove_styles = ["straight", "swing", "shuffle"]
        for gs in groove_styles:
            features.append(1.0 if query.groove_style == gs else 0.0)
    ...
    def _predict_family_ml(
        ...
        try:
            ...
            logger.error(f"Bass ML prediction failed: {exc}")
            return self._predict_family_rule_based(query)
    ...
    def _predict_family_rule_based(
        ...
        """
        # テンポ/セクション/グルーブからFamily推定
        if query.tempo_bpm < 70:
            family = "ballad_whole_note"
        elif query.tempo_bpm > 160:
            family = "fast_eighth_note"
        elif query.groove_style == "swing":
            family = "swing_walking"
        elif query.groove_style == "shuffle":
            family = "shuffle_triplet"
        elif query.section == "Chorus":
            family = "chorus_driving"
        elif query.chord_type in ["7", "maj7", "min7"]:
            family = "jazz_walking"
        else:
            family = "standard_quarter"
    ...
    def _select_pattern_from_family(
        ...
        if not candidates:
            logger.warning(f"No Bass patterns found for family: {family}")
            return None
        
        # Tempo/Section/Groove適合度でスコアリング
        scored = []
        for pid in candidates:
            ...
            tempo_score = max(0.0, 1.0 - tempo_diff / 40.0)
            ...
            # Groove適合度
            ptn_groove = ptn.get("groove_hint", "straight")
            groove_score = 1.0 if ptn_groove == query.groove_style else 0.6
    ...
    def _get_safe_kit_pattern(
        ...
        if not self.safe_kit:
            ...
            return {
                ...
                "family": "safe_kit",
                "events": self._generate_basic_root_pattern(query),
        ...
        # Tempo/Groove適合度でスコアリング
        scored = []
        for sp in safe_patterns:
            sp_tempo = sp.get("tempo_hint", 120)
            sp_groove = sp.get("groove_hint", "straight")
            
            tempo_diff = abs(query.tempo_bpm - sp_tempo)
            tempo_score = max(0.0, 1.0 - tempo_diff / 40.0)
            groove_score = 1.0 if sp_groove == query.groove_style else 0.5
            
            score = tempo_score * 0.6 + groove_score * 0.4
    ...
    def _generate_basic_root_pattern(
        self,
        query: BassQuery
    ) -> List[Dict[str, Any]]:
        """基本ルートパターン生成（緊急フォールバック）
        ...
        # Chord Rootをピッチに変換（ベースは1オクターブ下）
        root_pitches = {
            "C": 36, "D": 38, "E": 40, "F": 41,
            "G": 43, "A": 45, "B": 47
        }
        root_pitch = root_pitches.get(query.chord_root, 36)
    ...
    def recommend(
        ...
        if safety_triggered:
            ...
            logger.info(f"Bass Safe-Kit triggered: {safety_reason}")
            
            return BassRecommendResult(
                ...
                root_hit_rate=0.90,  # Safe-Kitはルート音中心で高め
                chord_fit=0.75,
                density=4.0,
            )
        ...
        if pattern_id is None:
            ...
            safe_pattern = self._get_safe_kit_pattern(query)
            logger.warning(f"No Bass pattern found for family: {top1_family}, using Safe-Kit")
        ...
        root_hit_rate = pattern.get("root_hit_rate", 0.85)
        chord_fit = pattern.get("chord_fit", 0.75)
        density = pattern.get("density", 6.0)
        
        logger.info(
            f"Recommended Bass pattern: {pattern_id} "
