...
class PatternRecommender:
    ...
    def _compute_statistics(self):
        ...
        self.techniques = set(p.metadata.technique for p in self.patterns)
        
        self.tempo_min = min(tempos)
        self.tempo_max = max(tempos)
    ...
    def recommend(
        ...
        for pattern in self.patterns:
            ...
            quality = pattern.metadata.score
        ...
        logger.debug(f"Recommended {len(results)} patterns (from {len(scored_patterns)} candidates)")
    ...
    def _calculate_similarity(
        ...
        # 1. Tempo similarity (必須、高ウェイト)
        tempo_score = self._tempo_similarity(query.tempo, pattern.metadata.tempo, query.tempo_tolerance)
