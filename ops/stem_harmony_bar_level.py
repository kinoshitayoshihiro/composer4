...
import os
os.environ.setdefault("NUMBA_DISABLE_JIT", "1")   # numbaのJITを完全オフ（遅くなるがBus errorを回避）
os.environ.setdefault("NUMBA_THREADING_LAYER", "workqueue")  # 競合回避（念のため）
os.environ.setdefault("NUMBA_CACHE_DIR", "/tmp/numba-cache")  # 壊れたキャッシュ回避
...
def sus4_template() -> np.ndarray:
    t = np.zeros(12, dtype=float); t[[0, 5, 7]] = 1.0; return t

def sus2_template() -> np.ndarray:
    t = np.zeros(12, dtype=float); t[[0, 2, 7]] = 1.0; return t

def add9_template() -> np.ndarray:
    t = np.zeros(12, dtype=float); t[[0, 2, 4, 7]] = 1.0; return t
...
_NAME_TO_PC = {"C":0,"C#":1,"Db":1,"D":2,"D#":3,"Eb":3,"E":4,"F":5,"F#":6,"Gb":6,
...
def _parse_priority(spec: str):
    ...
    out = []
    for t in tokens:
        if t == "sus": out.append(("sus", ["sus4","sus2"]))
        elif t in ("add9","6","sus4","sus2"): out.append((t,[t]))
    if not out:
...
def _scale_set(tonic_pc:int, mode:str):
    """キーのダイアトニックスケール（7音）"""
    if mode=="maj": return {(tonic_pc+i)%12 for i in (0,2,4,5,7,9,11)}
...
def decorate_events_with_bar_chroma_v3(
    ...
    base = dict(
        ...
        sus_tau_on=sus_tau_on, sus_tau_off=sus_tau_off,
        switch_margin=switch_margin, min_dwell_bars=min_dwell_bars,
        triad_guard=triad_guard, allow_sus=allow_sus
    )
    def cfg_for_section(name: Optional[str]):
        c = dict(base)
        if "default" in cfg_all: c.update(cfg_all["default"])
        if name in cfg_all:      c.update(cfg_all[name])
        return c
    ...
    def deco_score_map(r:int, q0:str, c:np.ndarray):
        ...
        if scale is not None:
            if deg2 not in scale: s_add9 -= key_bias_penalty; s_sus2 -= key_bias_penalty
            if deg4 not in scale: s_sus4 -= key_bias_penalty
            if ((r+9)%12) not in scale: s_6 -= key_bias_penalty
        
        return {"add9":s_add9, "6":s_6, "sus2":s_sus2, "sus4":s_sus4,
                "triad":triad_strength, "third":c[third_pc]}
    ...
    for ev in events:
        ...
        # 候補（ONしきい値）
        cand = {}
        if sc["add9"] >= float(cfg["tau_on"]): cand["add9"] = sc["add9"]
        if sc["6"]    >= float(cfg["tau_on"]): cand["6"]    = sc["6"]
        if cfg["allow_sus"]:
            if sc["sus4"] >= float(cfg["sus_tau_on"]): cand["sus4"] = sc["sus4"]
            if sc["sus2"] >= float(cfg["sus_tau_on"]): cand["sus2"] = sc["sus2"]
        ...
        # 最短持続
        if last_deco in ("add9","6","sus2","sus4") and (b - last_change_bar) < int(cfg["min_dwell_bars"]):
...
def recognize_chords_per_bar_dp(
    ...
    if key_bias is not None:
        ...
        def base_kind(q):
            q = q or ""
            if "min" in q: return "min"
            if "maj" in q or "sus" in q or "add" in q or q == "6": return "maj"
            return "maj"
    ...
    assert np.all((conf >= 0.0) & (conf <= 1.0)), f"confidence not in [0,1]: min={conf.min():.3f}, max={conf.max():.3f}"
...
def main():
    ...
    ap.add_argument("--change-penalty", type=float, default=0.15, help="DP change penalty (0.12-0.20 recommended)")
    ap.add_argument("--use-dp", action="store_true", help="Use DP-based recognition (recommended)")
    ap.add_argument("--triad-only", action="store_true", help="Use only major/minor triads (24 states), then decorate as needed")
    ...
    ap.add_argument("--decorate-priority", type=str, default="add9>6>sus",
    ...
    ap.add_argument("--dec-tau-off", type=float, default=None, help="add9/6 OFF threshold (default: tau_on-0.05)")
    ap.add_argument("--dec-sus-tau-on", type=float, default=0.35, help="sus ON threshold")
    ap.add_argument("--dec-sus-tau-off", type=float, default=None, help="sus OFF threshold (default: sus_tau_on-0.05)")
    ap.add_argument("--dec-switch-margin", type=float, default=0.08, help="Margin required for decoration switch")
    ap.add_argument("--dec-triad-guard", type=float, default=0.35, help="3rd threshold to block sus (higher → more triads)")
    ap.add_argument("--dec-density-window-bars", type=int, default=8, help="Sliding window size for density limiter")
    ...
    ap.add_argument("--dec-key-bias", type=str, default=None, help="Key bias (e.g., 'D:maj' or 'B:min')")
    ap.add_argument("--dec-key-bias-penalty", type=float, default=0.05, help="Penalty for non-diatonic decoration")
    ap.add_argument("--dec-config", type=str, default=None,
                    help="Section-specific thresholds JSON/YAML (default/Verse/Chorus...)")
    
    # ChatGPT推奨: triad vs 装飾のバランス調整
    ap.add_argument("--dec-triad-gain", type=float, default=0.75, help="Triad strength multiplier (0.65-0.85, lower → more decorations)")
    ...
    ap.add_argument("--dec-sus-third-coeff", type=float, default=0.80, help="sus 3rd subtraction coefficient (0.75-0.85)")
    ap.add_argument("--dec-decor-margin", type=float, default=-0.02, help="Decoration vs triad margin (-0.04 to 0.00)")
    ...
    def _compute_chroma():
        ...
        C_sync, tempo, beat_times = chroma_sync_safe(y_h, sr_mix, n_fft=4096, hop_length=512)
        return (C_sync, np.array([tempo]), beat_times, np.array([sr_mix]))
    
    cache_path = cache_dir_path / f"chroma_bar_{cache_key}.npz"
    if use_cache and cache_path.exists():
        ...
        print(f"[CACHE] HIT: chroma_bar_{cache_key[:8]}.npz")
    else:
        ...
        if use_cache:
            ...
            print(f"[CACHE] SAVE: chroma_bar_{cache_key[:8]}.npz")
    
    # QL mapper (labelerも取得)
    beat_to_ql, labeler = load_sections_mapper_and_labeler(
        sections_path,
        default_ql_per_beat=args.ql_per_bar / 4.0,
        beat_times=beat_times
    )
    
    # Convert beat times to QL
    beat_times_ql = np.array([beat_to_ql(i) for i in range(len(beat_times))])
    
    # Aggregate chroma by bars
    C_bars, bar_times = aggregate_chroma_by_bars(C_sync, beat_times_ql, ql_per_bar=args.ql_per_bar)
    ...
    n_states = len(labels)
    print(f"[INFO] Using {'triad-only' if args.triad_only else 'extended'} templates: {n_states} states")
    ...
    if args.force_key:
        ...
        # 伴奏ミックス C_sync から自動
        tonic_pc, mode = estimate_key_ks(C_sync)
        kb = (tonic_pc, mode)
    ...
    if args.decorate:
        ...
        if args.dec_key_bias:
            ...
            name = name.strip(); mode = mode.strip().lower()
            tonic = _NAME_TO_PC.get(name.upper(), 0)
            key_bias = (tonic, "maj" if mode.startswith("maj") else "min")
        elif kb:
            key_bias = kb  # 推定キーを使用
        
        # QL → section名 の関数（labelerが無い場合はNone返す）
        get_section = (lambda ql: labeler(ql) if callable(labeler) else None)
        
        events = decorate_events_with_bar_chroma_v3(
            events, C_bars, label_at_ql=get_section, ql_per_bar=args.ql_per_bar,
            tau_on=args.dec_tau_on, tau_off=args.dec_tau_off,
            sus_tau_on=args.dec_sus_tau_on, sus_tau_off=args.dec_sus_tau_off,
            switch_margin=args.dec_switch_margin, min_dwell_bars=args.min_dwell_bars,
            ...
            priority=args.decorate_priority, allow_sus=(not args.dec_no_sus),
            allow_m6=args.dec_allow_m6,
            key_bias=key_bias, key_bias_penalty=args.dec_key_bias_penalty,
        ...
        print(f"[INFO] Decoration applied: {triads} triads ({triads/len(events):.1%}), {extended} extended ({extended/len(events):.1%})")
    ...
    print(f"[INFO] Density: {len(events)} events / {len(bar_times)} bars = {len(events)/len(bar_times):.2f} chords/bar")
