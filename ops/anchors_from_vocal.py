...
def _sections_labeler_and_ql(sections_path: Optional[Path], beat_times: Optional[np.ndarray], default_ql_per_beat: float = 1.0):
    data = _safe_load_json(sections_path)
    secs = sorted(data.get("sections") or [], key=lambda s: float(s.get("time_sec", s.get("time", 0))))
    ...
    for s in secs:
        ...
        if "time_sec" in s: t = float(s["time_sec"]) 
        elif "time" in s:  t = float(s["time"]) 
        else: t = None
...
def _extract_candidates(y: np.ndarray, sr: int) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    onset_frames = librosa.util.peak_pick(oenv, pre_max=3, post_max=3, pre_avg=3, post_avg=5, delta=np.median(oenv)*0.5, wait=3)
    ...
    if len(oenv):
        ...
        ost_n = np.array([])
    # zcr
    zcr = librosa.feature.zero_crossing_rate(y=y, frame_length=int(0.025*sr), hop_length=int(0.010*sr))[0]
...
def _align_tokens_to_onsets(tokens: List[str], onset_times: np.ndarray) -> List[Tuple[float, Optional[str]]]:
    if not tokens:
...
def _load_windows(path: Optional[Path]) -> Dict[str, Dict[str, float]]:
    ...
    if path.suffix.lower() in (".yaml",".yml") and yaml:
        try:
            ...
            return {k:{"pre": float(v.get("pre",0)), "post": float(v.get("post",0))} for k,v in (d.get("windows",{}) or {}).items()} or _DEF_WINDOWS
        except Exception:
            return _DEF_WINDOWS
    else:
        try:
...
# ---------------------------- main ----------------------------
def main():
    ap = argparse.ArgumentParser(description="Extract lyric anchors from vocal stem (onset-based, optional lyric alignment)")
    ...
    # beat基準（拍長に対する比率）
    ap.add_argument("--beat-pre-frac", type=float, default=0.25, help="beat mode: pre as fraction of beat dur")
    ap.add_argument("--beat-post-frac", type=float, default=0.35, help="beat mode: post as fraction of beat dur")
    ...
    ap.add_argument("--energy-alpha", type=float, default=0.6, help="scale = 1 + alpha*(E-baseline)")
    ap.add_argument("--energy-baseline", type=float, default=0.5)
    ap.add_argument("--energy-win-ms", type=float, default=50.0)
    
    # sibilant強調/限定
    ap.add_argument("--sibilant-scale", type=float, default=1.0, help=">1.0 to enlarge sibilant windows")
    ap.add_argument("--sibilant-only", action="store_true", help="emit only anchors that include sibilant class")
    ...
    # sections + QL
    # ここでは beat_times は onsets と同程度の分解能があれば良い → onset抽出時と同Hopではなく、librosa.tempo+beatsを再推定して使う
    print(f"[INFO] Detecting beats...")
    tempo, beats = librosa.beat.beat_track(y=y, sr=sr, units='time', tightness=100)
    tempo_val = float(tempo[0]) if isinstance(tempo, np.ndarray) and len(tempo) > 0 else float(tempo)
    print(f"[INFO] Detected tempo: {tempo_val:.1f} BPM, {len(beats)} beats")
    
    label_at_sec, sec_to_ql = _sections_labeler_and_ql(Path(args.sections) if args.sections else None, np.array(beats) if len(beats) else None)
    ...
    for idx, ((t, tok), ost, z, c) in enumerate(zip(onset_pairs, onset_str_norm, z_at, cent_at)):
        ...
        if args.window_mode == "fixed":
            pre = float(args.fixed_pre if args.fixed_pre is not None else _DEF_WINDOWS["stress"]["pre"])
            post = float(args.fixed_post if args.fixed_post is not None else _DEF_WINDOWS["stress"]["post"])
            ...
            pre  = float(np.clip(1000.0 * gap_pre * float(args.prop_k_pre),  float(args.prop_min_ms), float(args.prop_max_ms)))
            post = float(np.clip(1000.0 * gap_post* float(args.prop_k_post), float(args.prop_min_ms), float(args.prop_max_ms)))
        ...
        anchors.append(Anchor(time=float(t), token=tok, classes=klasses, section=sec, time_ql=float(ql) if ql is not None else None, windows_ms=w))
